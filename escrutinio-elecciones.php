...
/**
 * 
 * @link              https://quierohacertuweb.com
 * @since             1.0.0
 * @package           dolar_blue_hoy
 * 
...
function mostrar_metabox_detalles($post) {
    ...
    $puesto = get_post_meta($post->ID, 'puesto', true); // Nuevo campo "Puesto"
    ...
    echo '<input type="text" id="partido" name="partido" value="' . esc_attr($partido) . '" /><br />';

    echo '<label for="votos">Votos (%):</label>';
    echo '<input type="number" id="votos" name="votos" step="0.01" min="0" max="100" value="' . esc_attr($votos) . '" /><br />';
